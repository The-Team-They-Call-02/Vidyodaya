    ...
    <Container>
      ...
      <CampCardContainer>
        ...
        <div className="camp-card-inner-container">
          {camp.title ? <h3 className="main-title"> {camp.title} </h3> : null}
          {camp.subtitle ? (
          ...
          {camp.villagesCovered === 0 ? (
            <h4 className="village-covered">
              {" "}
              Village Covered:{" "}
              <span className="count"> {camp.villagesCovered} </span>
            </h4>
          ) : null}
          {camp.description ? (
            <p className=" main-description"> {camp.description} </p>
          ) : null}
          <br />
          {camp.highlights ? (
              ...
              {camp.highlights.map((hl) => {
                ...
                return (
                  <div className="camp-card-highlights-inner-div">
                    {hl.highlightTitle ? (
                      <h4 className="title"> ∎ {hl.highlightTitle}</h4>
                    ) : null}
                    {hl.description ? (
                      <p className="description"> {hl.description} </p>
                    ) : null}
                    {hl.imageURL ? (
                      <img
                        src={hl.imageURL}
                        alt={hl.highlightTitle ? hl.highlightTitle : null}
                      />
                    ) : null}
                  </div>
                );
              ...
              {camp.feedback.map((fb) => {
                return fb.testimonial ? (
                  <p className="testimonial">
                    "{fb.testimonial}"{" "}
                    <span className="source"> -{fb.source} </span>
                  </p>
                ) : null;
              })}
            </div>
